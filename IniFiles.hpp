...
public:
	...
	bool getBoolean(std::string section_name, std::string key_name)
	{
		...
		if (!_stricmp("true", (char*)convToLowercase(szBuffer).c_str()) ||
			...
			!_stricmp("вкл", (char*)convToLowercase(szBuffer).c_str()) ||
			!_stricmp("включено", (char*)convToLowercase(szBuffer).c_str()) ||
			!_stricmp("включена", (char*)convToLowercase(szBuffer).c_str()) ||
			!_stricmp("включен", (char*)convToLowercase(szBuffer).c_str()) ||
			!_stricmp("да", (char*)convToLowercase(szBuffer).c_str()))
			return true;
		return false;
	}
	...
	void setBoolean(std::string section_name, std::string key_name, bool value, bool rus = false)
	{
		...
		if (value) WritePrivateProfileStringA(section_name.c_str(), key_name.c_str(), rus ? "да" : "true", getIniFileFullPath().c_str());
		else WritePrivateProfileStringA(section_name.c_str(), key_name.c_str(), rus ? "нет" : "false", getIniFileFullPath().c_str());
	}
